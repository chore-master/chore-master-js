...
import { Asset, Portfolio, UpdateTransferFormInputs } from '@/types/finance'
...
export default function UpdateTransferForm({
  portfolio,
  updateTransferForm,
  assetInputValue,
  isFetchingAssets,
  ...
  handleSubmitUpdateTransferForm: SubmitHandler<UpdateTransferFormInputs>
}) {
  const settlementAsset =
    assetReferenceToAssetMap[portfolio?.settlement_asset_reference as string]
  return (
    <Box sx={{ minWidth: 320 }}>
      <CardHeader title="編輯流量" />
      <Stack
        component="form"
        spacing={3}
        p={2}
      ...
      >
        <Controller
          name="flow_type"
          control={updateTransferForm.control}
          render={({ field }) => (
          ...
          <Controller
            ...
            render={({ field }) => (
              <TextField
                ...
                disabled={
                  updateTransferForm.watch('flow_type') === 'UPDATE_POSITION'
                }
              />
            )}
            rules={{ required: '必填' }}
        ...
        <AutoLoadingButton
          type="submit"
          variant="contained"
          disabled={!updateTransferForm.formState.isValid}
          onClick={updateTransferForm.handleSubmit(
            handleSubmitUpdateTransferForm
          )}
        >
          新增
