...
export const TimezoneProvider = ({ children }: Readonly<{ children: React.ReactNode }>) => {
  const deviceOffsetInMinutes = -new Date().getTimezoneOffset();
  const [offsetInMinutes, setOffsetInMinutes] = React.useState(deviceOffsetInMinutes);
  ...
  React.useEffect(() => {
    ...
    setOffsetText(`${sign}${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}`);
  }, [offsetInMinutes]);
